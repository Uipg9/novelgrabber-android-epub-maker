...
async function fetchChapterFast(url, config, options = {}) {
    ...
    try {
        ...
        // Inject and execute the fetch function in the tab's page context
        ...
        // Get the result from the injected function
        const result = results?.[0]?.result;
        
        if (!result) {
            throw new Error('No result from tab proxy - script injection may have failed');
        }
